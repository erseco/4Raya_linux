...
CURSOR_PUT4_BTN_ID=504, CURSOR_PUT5_BTN_ID=505, CURSOR_PUT6_BTN_ID=506, CURSOR_PUT7_BTN_ID=507;
const int ERROR_OK_BTN_ID=602;

int main_window;
char life_time_input[1000]="150.214.190.149",total_runs_input[1000]="Ernesto",file_name_input[1000]="150.214.190.149";
char display_step[20]="10",display_time[20]="10";
char ip_server[1000],mi_nick[1000],file_name_temp[1000];
int tx,ty,tw,th;
...
GLUI_Button *cursor_put1, *cursor_put2, *cursor_put3, *cursor_put4, *cursor_put5, *cursor_put6, *cursor_put7;
	...
	switch(modo){
	    case 2: // soy cliente
	            if (partida_remota and esperando_server)
 	              sout<<"Esperando la jugada del rival";
 	            else
 	              sout<<"Te toca jugar...";
                break;
	    case 3: // soy cliente
...
void GUI::control_cb(int id){
	switch(id){
		case SELECTMAP_BTN_ID:
			...
			strcpy(ip_server,life_time_input);
			strcpy(mi_nick,total_runs_input);
		...
		case NEWMAP_OK_BTN_ID:
			strcpy(life_time_input,ip_server);
			strcpy(total_runs_input,mi_nick);
			newGame();
			//new_map_btn->enable();
			select_map_btn->enable();
			new_map_glui->close();
		...
		case SELECTMAP_CANCEL_BTN_ID:
			...
			if (modo==2){ // En red yo en modo cliente
                ...
                Cliente.Establecer_Conexion(ip_server, "5126");
                ...
                if (jugador==0)
                   Servidor.Enviar_Cliente_Msg("1");
                else
                   Servidor.Enviar_Cliente_Msg("0");
                ...
                Cliente.Establecer_Conexion(file_name_input, "5126");
                cout << "Empezamos las partida..." << endl;
        ...
        case CURSOR_PUT7_BTN_ID:
             ...
             switch (modo){
                 ...
                 case 2: // Vs jugador remoto (yo cliente)
                         cout << "Jugador Activo: " << env->JugadorActivo() << "  jugador: " << jugador << endl;
                         if ((env->JugadorActivo()==1 and jugador==0) or (env->JugadorActivo()==2 and jugador==1)){
                              do_move(id-501);
                              char aux=(id-501)+48;
                              // Enviar Accion
