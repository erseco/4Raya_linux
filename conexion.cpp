...
void conexion_server::Enviar_Cliente_Msg( char *msg){
    if(send(conectado,(char*) msg,strlen(msg),0)<0){ // Mensaje bienvenida para jugador 1
...
void conexion_server::Enviar_Accion_al_Cliente(char accion){
...
void conexion_client::Recibir_Msg(char *msg){
    memset(buffer,0,sizeof(buffer));           //esta funcion vacia el buffer de entrada
    bytesrecv=recv(sock,buffer,1024,0);
    if(bytesrecv == -1){
